...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tab-desc { font-size: 13px; color: var(--text-secondary); }
        ...
        .btn { background: rgba(255,255,255,0.1); color: white; border: 1px solid var(--border); padding: 8px 12px; border-radius: 6px; cursor: pointer; font-family: var(--font-main); }
        ...
        .alt1-banner { background: rgba(56, 189, 248, 0.1); border: 1px solid var(--brand); padding: 10px; border-radius: 6px; margin-bottom: 12px; font-size: 13px; color: var(--brand); display: none; }
    </style>
</head>
<body>
    <div class="app-container">
        <header>
            ...
            <div class="header-controls">
                ...
                <button class="icon-btn" onclick="fetchPrices()" title="Refresh">↻</button>
        ...
        <div class="toolbar">
            <span class="tab-desc" id="tab-desc">Top 25 Most Profitable Items</span>
    ...
    <div class="modal-overlay" id="settings-modal" onclick="if(event.target===this) toggleSettings()">
        <div class="modal">
            ...
            <div class="form-group">
                ...
                <input type="number" id="setting-charge" class="form-input" placeholder="Live GE Price" onchange="saveSettings()">
            </div>
            ...
            <button class="btn" style="width:100%; margin-top:10px;" onclick="addCustomItem()">Add to Alchemiser</button>
    ...
    <script>
        ...
        const MACHINES = {
            "Alchemiser": { 
                2: { iph: 25, cpi: 150/25, cap: 10000 },
                1: { iph: 8, cpi: 55/8, cap: 5000 }
            },
            "Plank Maker": { 
                2: { iph: 40, cpi: 600/40, cap: 10000 },
                1: { iph: 40, cpi: 600/40, cap: 10000 } // Doesn't have Mk1 typically, fallback to same
            },
            "Hide Tanner": { 
                2: { iph: 140, cpi: 225/140, cap: 10000 },
                1: { iph: 140, cpi: 225/140, cap: 10000 }
            },
            "Potion Producer": { 
                2: { iph: 40, cpi: 210/40, cap: 10000 },
                1: { iph: 40, cpi: 210/40, cap: 10000 }
            },
            "Auto Disassembler": { 
                2: { iph: 35, cpi: 180/35, cap: 10000 },
                1: { iph: 20, cpi: 100/20, cap: 5000 }
            }
        };
        ...
        const COMPONENT_RECIPES = [
            {item: "Noxious scythe", component: "Noxious components", yield: 1},
            {item: "Noxious longbow", component: "Noxious components", yield: 1},
            {item: "Dragon rider lance", component: "Ilujankan components", yield: 1},
            {item: "Zaros godsword", component: "Ilujankan components", yield: 1},
            {item: "Hand cannon", component: "Explosive components", yield: 1},
        ];

        let state = { tab: 'Dashboard', sortBy: 'day', prices: null, masterData: [] };
        let settings = { tax: true, junkTier: 0, customCharge: null, tier: 2, customAlch: [] };

        function init() {
            const saved = localStorage.getItem('inv_margins_settings_v2');
        ...
        function saveSettings() {
            ...
            localStorage.setItem('inv_margins_settings_v2', JSON.stringify(settings));
        ...
        function removeCustomItem(idx) {
            settings.customAlch.splice(idx, 1);
            saveSettings();
        }
        ...
        function setTab(tabName) {
            ...
            document.querySelectorAll('.tab').forEach(btn => {
                btn.classList.toggle('active', btn.innerText === tabName || (tabName === 'Auto Disassembler' && btn.innerText === 'Disassembler'));
            });
            document.getElementById('tab-desc').innerText = tabName === 'Dashboard' ? 'Top 25 Most Profitable Items' : 
        ...
        function getWikiIconUrl(name) { return `https://runescape.wiki/images/${name.split(' -> ').pop().replace(/ /g, '_')}.png`; }
        function toggleCard(el) { el.classList.toggle('expanded'); }

        function scanBankAlt1() {
            if (window.alt1) {
                alert("Alt1 detected! Screen scanning API hook would execute here to read your bank slots and cross-reference with the masterData array.");
            } else {
                alert("Alt1 not detected. Please open this app inside Alt1 Toolkit to use native screen reading features.");
            }
        }
        ...
        function calculateData() {
            ...
            [...DEFAULT_ALCH_ITEMS, ...settings.customAlch].forEach(i => {
                ...
                master.push({
                    ...
                    bottleneck: (i.limit * 6) < alchDaily, limit: i.limit, iconName: i.name,
                    capital: p * alchDaily,
                    details: `Daily Capital Req: ${formatNum(p * alchDaily)}<br>Uptime: ${(MACHINES["Alchemiser"][t].cap / alchDaily).toFixed(1)} Days<br>Formula: ${formatNum(i.alch)} - ${formatNum(p)} - ${formatNum(natPrice)} - ${formatNum(MACHINES["Alchemiser"][t].cpi * cCost)}`
                });
            });
            ...
            PLANK_RECIPES.forEach(r => {
                ...
                master.push({
                    ...
                    bottleneck: (r.limit * 6) < plankDaily, limit: r.limit, iconName: r.plank,
                    capital: (logP + r.coin_cost) * plankDaily,
                    details: `Daily Capital Req: ${formatNum((logP + r.coin_cost) * plankDaily)}<br>Uptime: ${(MACHINES["Plank Maker"][t].cap / plankDaily).toFixed(1)} Days`
                });
            });
            ...
            TANNER_RECIPES.forEach(r => {
                ...
                master.push({
                    ...
                    bottleneck: (r.limit * 6) < tannerDaily, limit: r.limit, iconName: r.leather,
                    capital: (hideP + r.coin_cost) * tannerDaily,
                    details: `Daily Capital Req: ${formatNum((hideP + r.coin_cost) * tannerDaily)}<br>Uptime: ${(MACHINES["Hide Tanner"][t].cap / tannerDaily).toFixed(1)} Days`
                });
            });
            ...
            DISASSEMBLE_RECIPES.forEach(r => {
                ...
                master.push({
                    ...
                    bottleneck: (r.limit * 6) < (disDaily * itemsNeeded), limit: r.limit, iconName: r.output,
                    capital: itemP * (disDaily * itemsNeeded),
                    details: `Daily Capital Req: ${formatNum(itemP * (disDaily * itemsNeeded))}<br>Uptime: ${(MACHINES["Auto Disassembler"][t].cap / (disDaily * itemsNeeded)).toFixed(1)} Days`
                });
            });
            ...
            COMPONENT_RECIPES.forEach(r => {
                ...
                master.push({
                    ...
                    bottleneck: false, limit: 10, iconName: r.component,
                    capital: itemP,
                    details: `Implied Value: ${formatNum(itemP)} / ${r.yield} = ${formatNum(compVal)}`
                });
            });
        ...
        function renderData() {
                ...
                settings.customAlch.forEach((c, idx) => {
                    ...
                    d.innerHTML = `
                        <div class="card-header">
                            <div><strong>${c.name}</strong> (Alch: ${c.alch})</div>
                            <button class="icon-btn" onclick="removeCustomItem(${idx})">🗑️</button>
                        </div>
                    `;
            ...
            data.forEach((row, idx) => {
                ...
                card.innerHTML = `
                    <div class="card-header">
                        <div class="card-left">
                            <img class="item-icon" src="${getWikiIconUrl(row.iconName)}" onerror="this.style.opacity='0'">
                            <div class="item-info">
                    ...
                    <div class="card-details">${row.details}</div>
                `;
